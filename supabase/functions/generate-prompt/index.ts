...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const instruction = [
      ...
      'Crucial: do NOT mention their mistakes, do NOT make it a grammar drill, and do NOT quote the corrections. It must read as a genuine, engaging real-life situation (work, social, travel, opinion, storytelling, negotiation) that simply happens to invite that language.',
      'Pitch it at their level: it should require some nuance, opinion, narration, or persuasion — never a beginner "order a coffee" task, unless their errors were specifically about that register.',
      'HARD REQUIREMENT — VARIETY: the learner strongly dislikes repeated prompts. The SITUATION must feel genuinely new every single time. It is fine (good, even) for the underlying LANGUAGE SKILL it trains to repeat, but the surface scenario, setting, roles, and framing must be clearly different from anything they have seen. Reach for unusual, specific, vivid situations rather than the obvious textbook ones.',
